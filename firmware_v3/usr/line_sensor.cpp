...
void LineSensor::init()
{
    ...
    ADC1->CHSELR = this->channels_map[0];
...
uint32_t LineSensor::callback(int32_t value)
{
    ...
    {
        // toogle LED
        // turn on or off led, depending on state
        if (this->led_state == true)
        {
        ...
        // here we can process previous results, and adds some delay for led state change
        this->_processing();
    }
    // waiting states, optional, for led settling time
    ...
    {
        // store results to corresponding register
        if (this->led_state)
        {
        ...
        // we returns next channel for ADC to read
        this->channel++;    

        // reading done, reset state and channel for next round
    ...
    // return next channel for ADC to read
...
void LineSensor::_adc_init() 
{
    ...
    LL_ADC_SetChannelSamplingTime(ADC1, LL_ADC_CHANNEL_16, LL_ADC_SAMPLINGTIME_COMMON_1);
    LL_ADC_SetChannelSamplingTime(ADC1, LL_ADC_CHANNEL_15, LL_ADC_SAMPLINGTIME_COMMON_1);
    LL_ADC_SetChannelSamplingTime(ADC1, LL_ADC_CHANNEL_17, LL_ADC_SAMPLINGTIME_COMMON_1);
    LL_ADC_SetChannelSamplingTime(ADC1, LL_ADC_CHANNEL_10, LL_ADC_SAMPLINGTIME_COMMON_1);
